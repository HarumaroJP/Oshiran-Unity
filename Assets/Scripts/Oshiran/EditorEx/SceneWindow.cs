...
public class SceneWindow : EditorWindow {
    ...
    static Dictionary<string, bool> _scenePathDict = new Dictionary<string, bool>();

    //選択中のシーンのパスのList
    static List<string> _selectingScenePathList = new List<string>();

    //シーンユニットの名前
    static string _sceneUnitName = "";
    ...
    public static void Open() {
        SceneWindow.GetWindow<SceneWindow>(typeof(SceneWindow));
    }
    ...
    //装飾とタイトルを付けて、GUIを表示
    void OnGUIWithTitle(Action onGUIAction, string title) {
        EditorGUILayout.BeginVertical(GUI.skin.box);
        {
    ...
    void OnSettingSceneGUI() {
        for (int i = 0; i < SceneUnitSet.Instance.UnitNum; i++) {
            ...
            if (GUILayout.Button("読み込み", GUILayout.Width(70))) {
    ...
    void OnAllSceneGUI() {
        ...
        foreach (KeyValuePair<string, bool> pair in _scenePathDict) {
            EditorGUILayout.BeginHorizontal(GUI.skin.box, GUILayout.Width(30));

            EditorGUILayout.LabelField(Path.GetFileNameWithoutExtension(pair.Key), GUILayout.Width(90));
        ...
        // if (GUILayout.Button("シーン再取得、選択全解除", GUILayout.Width(160))) {
        //     Init();
        // }
    }
